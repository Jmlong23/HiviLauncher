...
- 屏保样式采用常见的 2 x 2 卡片布局：`简约时钟`、`天气时钟`、`相册轮播`、`夜间息屏`；默认选中`简约时钟`。不提供“音律”样式。
- 每张卡片采用 16:9 缩略预览，包含样式名称、简短说明和选中标识；选中态使用高亮边框 + 右上角勾选，不只依赖颜色区分。
...
| 相册轮播 | 一张横向照片/内置艺术图，底部弱化显示时间 | 图片以淡入淡出方式轮播，建议 20-30 秒切换一次；首发使用内置艺术图，后续可在取得用户授权后接入系统相册。 |
| 夜间息屏 | 黑色预览画面 + 月亮图标 | 原“黑屏”调整为“夜间息屏”：屏幕不显示时钟、图片或动效，仅保持纯黑低功耗显示，用于夜间防眩光；音频播放不受影响。 |

- 用户点击任一卡片即切换选中态并保存；点击`预览`前不需要二次确认。天气时钟不可用时卡片保持可选，但预览和实际效果均展示降级后的简约时钟。
