...
> 本文档以“设计师可以直接据此画页面”为目标。除非特别说明，文档中的音量、连接、播放状态均需要在界面上给出清晰的视觉反馈。硬件实现参数以最终声学和结构评审为准，UI 需预留状态、异常和多语言空间。
...
| 音效音量 | 混响、回声等人声效果强度 | 仅卡拉 OK 页面 | 页面内独立滑杆/展开面板 |
...
| 音效音量 | 调整混响、回声总强度；支持展开混响、回声的开关与档位 |

卡拉 OK 面板顶部显示“卡拉 OK 音量”，底部提供“恢复卡拉 OK 默认”按钮；此按钮仅重置话筒音量和音效音量，不重置全局功放音量。三条滑杆使用不同的辅助图标和说明，禁止只以颜色区分。
...
**页面目标**：直观看到蓝牙连接对象，完成连接、断开和复位。

页面区块：

1. 顶部连接卡：蓝牙图标、状态、设备名。
2. 操作按钮：`连接设备`、`断开连接`、`重置蓝牙`。
3. 播放信息：歌曲名、艺术家，歌词
4. 播放暂停，上一曲，下一曲按钮
...
| 音效 | `音效音量` | 调整混响、回声效果强度 | 页面内直接拖动滑杆；底部面板可展开混响/回声细项 |

音效细项仅在点击底部“音量”打开的三音量面板内展开：

- 混响：关闭、轻柔、标准、大空间（卡片单选）。
- 回声：关闭、低、中、高（卡片单选）。
- 音效音量总滑杆：作为混响和回声的总强度。
...
- 两支麦克风可分别显示音量时，页面的“话筒音量”调节区拆分为“话筒 1/话筒 2”两条滑杆；否则统一使用“话筒音量”。
...
2. 页面内展示搜索动画和设备列表。
3. 点击设备后显示“正在连接”。
4. 成功：设备卡变为已连接，顶部状态同步，频谱开始跳动。
5. 失败：保留页面，提示失败原因，提供“重试”。
...
4. 蓝牙连接、WiFi 配网、卡拉 OK 麦克风连接、恢复出厂的完整弹窗和时序。
5. FFT 频谱、音量气泡、输入切换、连接成功等关键动效说明。
6. 中文和 English 双语稿，检查按钮、状态、错误提示的长度适配。
7. 遥控器按键映射稿：音量、静音、输入切换、播放控制、卡拉 OK 常用操作。
8. 图标、颜色、字体、间距、圆角、阴影、动效时长等设计规范。
9. AI 粒子核心的空闲、聆听、思考、回答、未授权和异常 6 组动效稿，以及每组对应的底部文案。
10. 屏保样式卡片、预览态，以及日志上报的确认、上传中、成功、失败状态稿。
