...
- [ ] 已连接 WiFi 的网络项点击后，补充确认弹窗：提供“断开连接”和“取消保存”操作，并分别说明其影响；用户取消时保持当前连接和已保存网络不变。
- [ ] 汽水音乐
- [ ] 麦克风改名字
- [ ] ai正在听悬浮设计
- [ ] 恢复出厂设置弹框，换掉进度条
...
底部系统导航栏是设备 UI 的全局一级导航，不随页面内容滚动，在全部产品页面始终显示相同的六项入口。其作用是让用户在任意页面都能一键返回上级、回到首页、查看最近任务、调节音量、打开应用或进入设置；输入模式通过顶部右侧“当前模式”区域切换。

导航栏紧贴屏幕底部安全区。音量不再常驻占据屏幕空间，而由“音量”导航项点击后以底部抽屉弹出。无论当前是有线、蓝牙、光纤、同轴、HDMI、WiFi 音乐、卡拉 OK、AI、授权或设置页面，底部系统导航栏均固定展示六项。

| 导航项 | 图标建议 | 页面定位 | 选中状态 |
| --- | --- | --- | --- |
| 返回 | Back / Chevron Left | 返回上一级页面 | 二级及以上页面可用；首页根页面置灰 |
| 首页 | Home | 七种输入模式入口、状态总览 | 填充图标 + 高亮色 |
| 后台 | Recent / History | 查看最近显示的任务 | 面板打开时高亮；英文标注为 `Recent` |
| 音量 | Volume / Speaker | 弹出当前模式对应的音量调节面板 | 点击态高亮；面板打开时保持高亮 |
| 应用 | Apps / Grid | 查看并启动设备已安装的系统应用 | 全部页面可用；打开时高亮 |
...
| 点击“后台” | 打开最近任务面板，展示最近显示的任务；点击任务恢复其页面或应用状态 |
| 点击“音量” | 弹出音量调节底部抽屉；普通模式显示功放音量，卡拉 OK 显示功放、话筒、音效三种音量 |
| 点击“应用” | 打开已安装系统应用列表；退出列表后返回进入前的页面 |
...
“后台”的英文标注为 `Recent`，用于展示用户最近显示过的任务。任务可以是最近访问的输入模式详情页、AI 对话、授权页、设置二级页或已启动的系统应用；不等同于 Android 系统设置中的后台进程管理。

- 最近任务面板采用横向或纵向卡片列表；单项显示任务图标、名称、最近显示时间和当前状态。
- 点击任一任务卡片后恢复到对应页面；对于系统应用，尝试恢复其最近任务或重新启动应用。
- 最近任务为空时显示空态：`暂无最近任务`。
- 任务按最近显示时间倒序排列；最多保留数量和清除策略由系统实现确认。

#### 3.1.5 应用

“应用”是全部页面常驻的系统应用入口，用于浏览并启动设备内已安装的系统应用。

- 应用列表为全屏页面，顶部显示“应用”和返回入口；底部六项导航保持可见。
- 应用按图标网格排列，单项展示应用图标与名称，支持上下滚动和遥控器焦点。
- 支持显示音乐类应用的“音乐”分类标签；其他系统应用归入“全部”。
- 无已安装应用时显示空态：`暂无可用应用`。
- 从应用列表返回时，回到进入应用列表前的页面，并保留原页面状态。
...
graph TD
    ...
    Recent[底部后台 Recent] --> RecentTasks[最近显示的任务]
    Apps[底部应用] --> AppList[系统应用列表]
    TopMode[顶部当前模式] --> Line
    TopMode --> BT
    TopMode --> Optical
    TopMode --> Coaxial
...
9. 底部系统导航栏：返回、首页、后台（Recent）、音量、应用、设置，全部页面保持一致。
...
**页面目的**：让用户使用手机扫码完成账号授权，使 WiFi 音乐、AI 等需要账号的能力获得授权状态。页面顶部保留返回按钮，底部六项系统导航栏保持显示。
...
7. 底部保持统一的六项系统导航栏；`应用`入口可随时查看全部已安装系统应用。
...
- 点击底部“应用”后，打开全屏系统应用列表；列表按应用图标、名称显示，支持上下滚动并返回进入前页面。
- 应用列表内点击音乐 App：启动对应 App；点击其他系统 App：启动后由 Android 系统承接，返回时回到进入应用列表前的页面。
...
- 屏保样式采用横向三卡片布局：`简约时钟`、`天气时钟`、`夜间息屏`；默认选中`简约时钟`，只提供上述三种样式。
- 每张样式卡片采用 16:9 缩略预览，包含样式名称、简短说明和选中标识；选中态使用高亮边框 + 右上角勾选，不只依赖颜色区分。
- 选择`简约时钟`或`天气时钟`后，在卡片下方显示“内置屏保”横向选择区；首发提供 6 套预置屏保背景：`深海蓝`、`晨雾灰`、`山峦绿`、`暖阳橙`、`星空紫`、`极夜黑`。内置屏保缩略图显示名称和选中标识，默认选中`深海蓝`；不提供图片上传或自动轮播。

| 项目 | 缩略预览内容 | 全屏表现与规则 |
| --- | --- | --- |
| 简约时钟（默认） | 大号时间、日期、星期，深色纯净背景 | 时间为主视觉；内容每分钟刷新，时钟位置可缓慢轻微位移，降低烧屏风险。 |
| 天气时钟 | 时间、天气图标、当前温度和城市名 | 适合客厅日常查看；需要网络和城市信息。无网络或天气不可用时，自动降级为简约时钟，并显示轻提示`天气暂不可用`。 |
| 夜间息屏 | 黑色预览画面 + 月亮图标 | 原“黑屏”调整为“夜间息屏”：屏幕不显示时钟、图片或动效，仅保持纯黑低功耗显示，用于夜间防眩光；音频播放不受影响。 |
| 内置屏保 | 6 套预置屏保背景缩略图 | 仅作用于简约时钟和天气时钟的背景；切换后立即保存，夜间息屏不显示内置屏保选择区。 |

- 用户点击任一屏保样式或内置屏保即切换选中态并保存；点击`预览`前不需要二次确认。天气时钟不可用时卡片保持可选，但预览和实际效果均展示降级后的简约时钟和当前已选内置屏保。
- 亮度：提供低/中/高三档或滑杆；屏保状态下应降低亮度，避免长时间强光和烧屏。
- 点击`预览`后全屏展示所选屏保；点击屏幕、遥控器任意键或进入需要用户确认的系统弹窗后退出预览。音频播放、音量变化和普通状态更新不退出屏保。
...
1. 横屏首页、七种输入详情页、顶部当前模式选择、六项底部导航、后台（Recent）最近任务、应用列表、二维码授权、AI 对话、设置全流程。
...
9. 屏保样式卡片、内置屏保卡片、预览态，以及日志上报的确认、上传中、成功、失败状态稿。
...
- 任意页面均固定显示返回、首页、后台（Recent）、音量、应用、设置六项底部导航；不因 WiFi 音乐模式改变导航项。
- “后台（Recent）”仅显示最近展示的任务；“应用”始终打开系统应用列表。
