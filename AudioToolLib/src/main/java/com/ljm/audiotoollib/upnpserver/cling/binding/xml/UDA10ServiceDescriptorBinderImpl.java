...
import static com.ljm.audiotoollib.upnpserver.cling.binding.xml.Descriptor.Service.ATTRIBUTE;
import static com.ljm.audiotoollib.upnpserver.cling.binding.xml.Descriptor.Service.ELEMENT;
...
import com.ljm.audiotoollib.upnpserver.cling.binding.staging.MutableAction;
import com.ljm.audiotoollib.upnpserver.cling.binding.staging.MutableActionArgument;
import com.ljm.audiotoollib.upnpserver.cling.binding.staging.MutableAllowedValueRange;
import com.ljm.audiotoollib.upnpserver.cling.binding.staging.MutableService;
import com.ljm.audiotoollib.upnpserver.cling.binding.staging.MutableStateVariable;
...
import com.ljm.audiotoollib.upnpserver.cling.model.meta.Action;
import com.ljm.audiotoollib.upnpserver.cling.model.meta.ActionArgument;
import com.ljm.audiotoollib.upnpserver.cling.model.meta.QueryStateVariableAction;
...
import com.ljm.audiotoollib.upnpserver.cling.model.meta.StateVariable;
import com.ljm.audiotoollib.upnpserver.cling.model.meta.StateVariableEventDetails;
import com.ljm.audiotoollib.upnpserver.cling.model.types.CustomDatatype;
import com.ljm.audiotoollib.upnpserver.cling.model.types.Datatype;
...
public class UDA10ServiceDescriptorBinderImpl implements ServiceDescriptorBinder, ErrorHandler {
    ...
    */
    ...
    private void generateSpecVersion(Service serviceModel, Document descriptor, Element rootElement) {
        ...
        XMLUtil.appendNewElementIfNotNull(descriptor, specVersionElement, ELEMENT.major, serviceModel.getDevice().getVersion().getMajor());
        XMLUtil.appendNewElementIfNotNull(descriptor, specVersionElement, ELEMENT.minor, serviceModel.getDevice().getVersion().getMinor());
    }

    private void generateActionList(Service serviceModel, Document descriptor, Element scpdElement) {

        Element actionListElement = XMLUtil.appendNewElement(descriptor, scpdElement, ELEMENT.actionList);
    ...
    private void generateAction(Action action, Document descriptor, Element actionListElement) {
        ...
        Element actionElement = XMLUtil.appendNewElement(descriptor, actionListElement, ELEMENT.action);

        XMLUtil.appendNewElementIfNotNull(descriptor, actionElement, ELEMENT.name, action.getName());

        if (action.hasArguments()) {
            Element argumentListElement = XMLUtil.appendNewElement(descriptor, actionElement, ELEMENT.argumentList);
    ...
    private void generateActionArgument(ActionArgument actionArgument, Document descriptor, Element actionElement) {
        ...
        Element actionArgumentElement = XMLUtil.appendNewElement(descriptor, actionElement, ELEMENT.argument);

        XMLUtil.appendNewElementIfNotNull(descriptor, actionArgumentElement, ELEMENT.name, actionArgument.getName());
        XMLUtil.appendNewElementIfNotNull(descriptor, actionArgumentElement, ELEMENT.direction, actionArgument.getDirection().toString().toLowerCase(Locale.ROOT));
...
//            log.warning("UPnP specification violation: Not producing <retval> element to be compatible with WMP12: " + actionArgument);
        ...
        XMLUtil.appendNewElementIfNotNull(descriptor, actionArgumentElement, ELEMENT.relatedStateVariable, actionArgument.getRelatedStateVariableName());
    }

    private void generateServiceStateTable(Service serviceModel, Document descriptor, Element scpdElement) {

        Element serviceStateTableElement = XMLUtil.appendNewElement(descriptor, scpdElement, ELEMENT.serviceStateTable);
    ...
    private void generateStateVariable(StateVariable stateVariable, Document descriptor, Element serviveStateTableElement) {
        ...
        Element stateVariableElement = XMLUtil.appendNewElement(descriptor, serviveStateTableElement, ELEMENT.stateVariable);

        XMLUtil.appendNewElementIfNotNull(descriptor, stateVariableElement, ELEMENT.name, stateVariable.getName());

        if (stateVariable.getTypeDetails().getDatatype() instanceof CustomDatatype) {
            XMLUtil.appendNewElementIfNotNull(descriptor, stateVariableElement, ELEMENT.dataType,
        ...
        XMLUtil.appendNewElementIfNotNull(descriptor, stateVariableElement, ELEMENT.defaultValue,
        ...
        if (stateVariable.getTypeDetails().getAllowedValues() != null) {
            Element allowedValueListElement = XMLUtil.appendNewElement(descriptor, stateVariableElement, ELEMENT.allowedValueList);
            for (String allowedValue : stateVariable.getTypeDetails().getAllowedValues()) {
                XMLUtil.appendNewElementIfNotNull(descriptor, allowedValueListElement, ELEMENT.allowedValue, allowedValue);
            }
        }

        if (stateVariable.getTypeDetails().getAllowedValueRange() != null) {
            Element allowedValueRangeElement = XMLUtil.appendNewElement(descriptor, stateVariableElement, ELEMENT.allowedValueRange);
