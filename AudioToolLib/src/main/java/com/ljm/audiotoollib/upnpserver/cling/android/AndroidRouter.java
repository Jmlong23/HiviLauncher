...
import com.ljm.audiotoollib.upnpserver.cling.transport.RouterImpl;
import com.ljm.audiotoollib.upnpserver.cling.transport.spi.InitializationException;
...
public class AndroidRouter extends RouterImpl {
    ...
    protected int lastKnownWifiIpAddress;
    ...
    public AndroidRouter(UpnpServiceConfiguration configuration,
        ...
        this.lastKnownWifiIpAddress = getCurrentWifiIpAddress();
    ...
    protected int getCurrentWifiIpAddress() {
        try {
    ...
    protected void onNetworkTypeChange(NetworkInfo oldNetwork, NetworkInfo newNetwork) throws RouterException {
        ...
        lastKnownWifiIpAddress = NetworkUtils.isWifi(newNetwork) ? getCurrentWifiIpAddress() : 0;
        if (enable()) {
    ...
    class ConnectivityBroadcastReceiver extends BroadcastReceiver {
        ...
        protected boolean isSameNetworkType(NetworkInfo network1, NetworkInfo network2) {
            ...
            if (network1 == null || network2 == null)
                return false;
            if (NetworkUtils.isWifi(network1) && NetworkUtils.isWifi(network2)) {
                int currentWifiIpAddress = getCurrentWifiIpAddress();
                if (currentWifiIpAddress != 0 && currentWifiIpAddress != lastKnownWifiIpAddress) {
                    log.info("WiFi IP changed, forcing router reconfiguration");
                    return false;
                }
            }
